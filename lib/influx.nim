...
proc serializeLine(line_data: LineProtocol, data_dict:Table[string,string]): string =
  ...
  for key, value in line_data.fields:
    ...
    if data_dict.hasKey(key):
        let dtype = data_dict[key]
        case dtype
        of "int":
...
proc write*(influx: InfluxDB, database: string, lineProtocolData: seq[LineProtocol], data_dict:Table[string,string]): (InfluxStatus, JsonNode) = 
  ...
  for item in lineProtocolData:
    let serialized_data = item.serializeLine(data_dict)
